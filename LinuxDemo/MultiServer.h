...
private:
	...
	evbuffer *m_ReadBuf;	// 读数据缓冲区
	evbuffer *m_WriteBUF;	// 写数据缓冲区
...
public:
	...
	// 获取可读数据的长度
	int GetReadBufferLen() { return evbuffer_get_length(m_ReadBuf); }
	...
	// 从读缓冲区中复制出len个字节的数据，存入buffer中，若不够，则复制出所有的数据
	// 返回复制出数据的字节数
	...
	// 将数据加入写缓冲区，准备发送
	int AddToWriteBuffer(char *buffer, int len) { return evbuffer_add(m_WriteBUF, buffer, len); }

	// 将读缓冲区中的数据移动到写缓冲区
	void MoveBufferData() { evbuffer_add_buffer(m_WriteBUF, m_ReadBuf); }
...
struct LibeventThread
{
	...
	int notifyReceiveFd;		// 管理的接受端
	int notifySendFd;			// 管理的发送端
	ConnQueue connectQueue;		// socket连接的链表

	// 在libevent的事件处理中要用到很多回调函数，不能使用类隐含的this指针
	// 所以用这样的方式将TcpBaseServer的类指针传过去
...
private:
	...
	static void CloseEventCb(struct buffereventf *bev, short events, void *data);
...
public:
	...
	event *AddTimerEvent(void(*prt)(int, short, void*), timeval tv, bool once);
